...
func TestRunRawHTMLColumnAlignItemsCenterPNG(t *testing.T) {
	t.Parallel()

	inputHTML := `<!DOCTYPE html><style>
#header-area {
  width: 100%;
  display: flex;
...
<div id="header-area">
  ...
  <div id="header-subtitle">Test Subheader</div>
	...
	cmd := runCommand(t, "--width", "500", "--format", "png", "--html", inputHTML,
		"-o", filepath.Join(t.TempDir(), "out.png"))
	cmd.Image.SmartWidth = false
	...
	if got := img.Bounds().Dx(); got != 500 {
		t.Fatalf("png width = %d, want 500", got)
	}
	...
	for _, testCase := range []struct {
		...
		center := float64(bounds.Min.X+bounds.Max.X) / 2
		if math.Abs(center-250) > 1 {
			t.Fatalf("%s center x = %.2f, want 250", testCase.name, center)
		}
	}
}
