...
type engine struct {
	...
	imgMaxW float64
	// imgCBW is the containing block content width for one in-flow replaced
	// element (block flow, flex/grid item). Percent and calc() widths resolve
	// against it; 0 means "not set" and the resolver falls back to imgMaxW or
	// the viewport. It is scoped to a single replaced build, so it never
	// leaks into inline collection.
	imgCBW float64
...
type box struct {
	...
	// escapeMarginBottom is a last-child bottom margin that collapsed through
	// this borderless auto-height box (CSS 2.1 8.3.1). layoutBlockChild folds
	// it into the outgoing sibling margin; it is not part of height.
	escapeMarginBottom float64
