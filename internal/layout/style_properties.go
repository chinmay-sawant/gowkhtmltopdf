...
func applyPositionGroup(
	...
	case cssVerticalAlignTop:
		setInset(style, cssVerticalAlignTop, value, fsize, ctx.viewportH)
	case floatRight:
		setInset(style, floatRight, value, fsize, ctx.viewportW)
	case cssVerticalAlignBottom:
		setInset(style, cssVerticalAlignBottom, value, fsize, ctx.viewportH)
	case floatLeft:
		setInset(style, floatLeft, value, fsize, ctx.viewportW)
	default:
		return false
	}
...
// setInset assigns one physical side from a top/right/bottom/left value.
// Percentages are stored for layout-time resolution against the containing
// block; fixed lengths resolve now against containing (viewport only for
// vw/vh-like units inside marginLen).
func setInset(style *ResolvedStyle, side, value string, fsize, containing float64) {
	fixedPt, pct, auto := insetValue(value, fsize, containing)

	switch side {
	case cssVerticalAlignTop:
		style.Top, style.TopPercent, style.TopAuto = fixedPt, pct, auto
	case floatRight:
		style.Right, style.RightPercent, style.RightAuto = fixedPt, pct, auto
	case cssVerticalAlignBottom:
		style.Bottom, style.BottomPercent, style.BottomAuto = fixedPt, pct, auto
	case floatLeft:
		style.Left, style.LeftPercent, style.LeftAuto = fixedPt, pct, auto
	}
}
...
func setWidthValue(style *ResolvedStyle, value string, fsize float64, ctx *styleContext) bool {
	if value == overflowAuto {
		...
		style.WidthCalc = false
		style.WidthCalcFixed = 0
	} else if pct, fixed, ok := calcLengthParts(value, fsize); ok && pct != 0 {
		// width:calc() with a percentage term: defer to layout so the
		// percentage resolves against the containing block, not the viewport.
		style.WidthCalc = true
		style.WidthCalcFixed = fixed
...
func assignLogicalInset(style *ResolvedStyle, prop, value string, fsize, viewportW, viewportH float64) {
	...
	case cssPropInsetBlock:
		start, end, parsed := logicalPair(value)
		if parsed {
			setInset(style, cssVerticalAlignTop, start, fsize, viewportH)
			setInset(style, cssVerticalAlignBottom, end, fsize, viewportH)
		}
	case cssPropInsetInline:
		start, end, parsed := logicalPair(value)
		if parsed {
			setInset(style, floatLeft, start, fsize, viewportW)
			setInset(style, floatRight, end, fsize, viewportW)
		}
	case "inset-block-start":
		setInset(style, cssVerticalAlignTop, value, fsize, viewportH)
	case "inset-block-end":
		setInset(style, cssVerticalAlignBottom, value, fsize, viewportH)
	case "inset-inline-start":
		setInset(style, floatLeft, value, fsize, viewportW)
	case "inset-inline-end":
		setInset(style, floatRight, value, fsize, viewportW)
	}
}
...
func applyInsetShorthand(style *ResolvedStyle, value string, fsize, viewportW, viewportH float64) {
	...
	setInset(style, cssVerticalAlignTop, top, fsize, viewportH)
	setInset(style, floatRight, right, fsize, viewportW)
	setInset(style, cssVerticalAlignBottom, bottom, fsize, viewportH)
	setInset(style, floatLeft, left, fsize, viewportW)
}
