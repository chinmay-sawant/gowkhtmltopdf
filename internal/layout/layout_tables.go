...
func (e *engine) layoutTableGrid(
	...
	if style.BGColor[3] > 0 && e.backgroundPaintEnabled(&style) && !hidesPaint(&style) {
...
func (e *engine) emitCell(cell *box, skipBorders bool) {
	...
	visible := !hidesPaint(&sty)

	if e.backgroundPaintEnabled(&sty) && !hideEmpty && visible {
	...
	if !skipBorders && !hideEmpty && visible {
