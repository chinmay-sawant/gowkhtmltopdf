...
func TestJustifyDoesNotGapBeforePunctuation(t *testing.T) { //nolint:gocognit,gocyclo,cyclop,funlen
	t.Parallel()
...
`)
	src := `<html><body><p style="width:420pt">` +
		`born 30 April 1988)<sup class="reference"><a href="#c1">` +
		`<span class="cite-bracket">[</span>1<span class="cite-bracket">]</span></a></sup> ` +
		`is a Cuban-born actress<sup class="reference"><a href="#c2">` +
		`<span class="cite-bracket">[</span>2<span class="cite-bracket">]</span></a></sup> ` +
		`holding Cuban,<sup class="reference"><a href="#c2b">` +
		`<span class="cite-bracket">[</span>2<span class="cite-bracket">]</span></a></sup> ` +
		`Spanish, and American citizenship.` +
		` including an <a href="/Academy_Award">Academy Award</a>, ` +
		`an <a href="/Actor">Actor Award</a>, ` +
		`a <a href="/BAFTA">BAFTA</a>, and two <a href="/Golden_Globe">Golden Globes</a>.` +
		` After moving to <a href="/LA">Los Angeles</a>, de Armas had English-speaking roles.` +
		` She starred opposite <a href="/r">Keanu Reeves</a> in her first Hollywood release—` +
		`<a href="/er">Eli Roth</a>'s erotic thriller.` +
		`</p></body></html>`
	...
	res, err := Layout(root, Options{ //nolint:exhaustruct // intentional zero fields
		Width: 420, Height: 800, Sheets: []*css.Stylesheet{cssSheet}, Background: true,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCiteDoesNotSplitAcrossLines(t *testing.T) { //nolint:cyclop
	t.Parallel()
...
`)
	src := `<html><body><p>` +
		`She often did not know what she was saying.` +
		`<sup class="reference"><a href="#c">` +
		`<span class="cite-bracket">[</span>37<span class="cite-bracket">]</span></a></sup> ` +
		`She spent four months learning English afterward with more words here to continue.` +
		`</p></body></html>`
	...
	res, err := Layout(root, Options{ //nolint:exhaustruct // intentional zero fields
		Width: 200, Height: 800, Sheets: []*css.Stylesheet{cssSheet}, Background: true,
	})
	if err != nil {
		t.Fatal(err)
	}
