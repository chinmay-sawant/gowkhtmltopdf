...
func TestBorderImageStretchUsesSlicedBorderOps(t *testing.T) {
	...
	eng := &engine{ //nolint:exhaustruct // image resolver only
	...
	sty := ResolvedStyle{ //nolint:exhaustruct // border-image fields under test
	...
	minX, minY, maxX, maxY := ops[0].X, ops[0].Y, ops[0].X+ops[0].W, ops[0].Y+ops[0].H
	sourceSizes := map[[2]int]int{}
	for _, op := range ops {
		if op.Kind != OpImage || !op.IsBackground {
			t.Fatalf("border-image op = %+v, want background image", op)
		}

		cfg, err := png.DecodeConfig(bytes.NewReader(op.Image))
		if err != nil {
	...
	if sourceSizes[[2]int{1, 1}] != 4 || sourceSizes[[2]int{2, 1}] != 2 || sourceSizes[[2]int{1, 2}] != 2 {
		t.Fatalf("border-image source slices = %v, want four corners, two horizontal edges, two vertical edges", sourceSizes)
	}
...
func TestBorderImageUsesPaintWidthForContent(t *testing.T) {
	...
	res := layoutHTML(t, `<html><body style="font-size:9.5pt;">
<div style="box-sizing:border-box;border:6px solid transparent;border-image-source:url(border.png);border-image-slice:1;padding:8px;width:90px;">img border</div>
</body></html>`)
