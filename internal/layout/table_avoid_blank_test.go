...
`)
	...
	var rows string
	for i := range 40 {
		rows += fmt.Sprintf(`<tr><td>%d</td><td>Title number %d with some words</td>`+
			`<td>Role %d</td><td>Original Spanish title Something Something</td></tr>`, 2000+i, i, i)
	}

	htmlSrc := `<html><body><h2>Film</h2>` +
		`<table><tr><th>Year</th><th>Title</th><th>Role</th><th>Notes</th></tr>` + rows + `</table></body></html>`
...
func TestAvoidTableAfterContentNoBlankCascade(t *testing.T) { //nolint:cyclop,funlen
	t.Parallel()
...
`)
	...
	var paras string
	for i := range 25 {
		paras += fmt.Sprintf(`<p>Lead paragraph %d with enough words to fill lines beside the upcoming filmography table section.</p>`, i) //nolint:lll // fixture copy
	}

	var rows string
	for i := range 18 {
		rows += fmt.Sprintf(`<tr><td>%d</td><td>Virgin Rose film title %d</td><td>Marie</td>`+
			`<td>Original Spanish title Una rosa de Francia long notes</td></tr>`, 2006+i, i)
	}
