...
wkhtmltopdf process comparison. `make bench-inprocess` is a separate Go
allocation matrix and does not measure an external process.
...
- They are not the in-process Go `B/op` or `allocs/op` matrix; those values
  belong to `make bench-inprocess` and `benchmark-results.txt`.
