...
|---|---|---|
| wkhtmltopdf | `make bench-cli-compare` → `TestCompareWithWkhtmltopdfBinary` | [`cli-compare.md`](cli-compare.md), [`cli-compare-results.csv`](cli-compare-results.csv) |
...
`make bench` means WeasyPrint and Puppeteer only. It never invokes
wkhtmltopdf. `make bench-cli-compare` is the only Makefile target for the
wkhtmltopdf process comparison. `make bench-engine` is the internal conversion
matrix; `make bench-lib` is the public API matrix. Neither measures an external
process.
...
directly. It constructs public documents from in-memory HTML before the timer,
then measures public validation, public-to-engine mapping, loading, layout,
painting, and PDF/PNG encoding. It does not start a CLI or read an HTML file
from disk. The benchmark does not include application-side template expansion;
that remains a separate concern from the library conversion API.
