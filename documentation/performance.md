...
- Consolidated current capture: [`documentation/benchmarks.md`](benchmarks.md)
...
CLI and external rows are the median of 3 timed runs after one warmup, wall
time via `/usr/bin/time`, peak RSS via `%M`; Puppeteer samples the process
tree. In-process rows ran `go test -benchmem -benchtime=1x -count=1`, one
fresh process per round; each value is the median of three rounds and `B/op`
and `allocs/op` are the median-time sample's raw values, never averages.
Reproduce: `make build`, `make bench-cli-compare`, `./scripts/bench-external.sh`,
and `./scripts/bench-performance-recovery.sh --mode=<mode>`. Method, raw
artifacts, and this same capture:
[`documentation/benchmarks.md`](benchmarks.md). Raw local captures:
`plans/0.2.6/perf-review/results/2026-09-12/`.

### CLI vs wkhtmltopdf (2026-09-12)
...
### CLI process, gowk only (2026-09-12 cli-rss)
...
### External engines (2026-09-12, 2 / 10 / 50 / 100 pages)
...
### In-process engine (internal, generic path, 2026-09-12)
...
### Public Go library PDF (2026-09-12)
...
### Public Go library image (2026-09-12)
...
go1.26.4. Only the generic path was measured.
...
The committed reference for the current capture is
[`documentation/benchmarks.md`](benchmarks.md); this page keeps the long
record and the dated earlier captures.
...
./scripts/bench-performance-recovery.sh --mode=<mode>   # internal-pdf-warm, internal-pdf, public-pdf, public-image, cli-rss
```
