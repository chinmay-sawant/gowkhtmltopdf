...
| `font-family` (named + generic) | Implemented | parsed + inherited; embedded Liberation Sans family (R/B/I/BI) plus **font registry** (`--font-path`, optional `--use-system-fonts`) and `@font-face` TTF/OTF/WOFF1/WOFF2 (local and `https://` via `FetchSub`) on **PDF and image** paths (see §4 / §5). Named families resolve as named; missing faces fall through the author stack, then Liberation; only CSS generics (`serif`/`sans-serif`/`monospace`) expand to Liberation |
...
| `@font-face` | Partial | Parsed; `MergeFontFaces` loads TTF/OTF/WOFF1/WOFF2 via `FetchSub` (local **and** `https://`) under the same ACL + `NetworkPolicy` on PDF and image paths. `.eot` sources are skipped; `data:` payloads register when the embedded format is WOFF2/WOFF1/TTF/OTF. TrueType outlines only. See §5 |
...
| `@font-face` (EOT / SVG fonts) | **Not supported:** `.eot` sources are skipped by policy; SVG font payloads fail parsing. WOFF2/WOFF1/TTF/OTF load via `FetchSub` (local **and** `https://`) under the same ACL + `NetworkPolicy` on PDF/image paths; `data:` payloads load when the embedded format is supported. Missing faces fall back to registry / Liberation |
