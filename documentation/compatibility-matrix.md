...
| Flexbox / Grid | Partial | **Flex Stage A:** `row\|column\|*-reverse`; wrap; `justify-content` incl. space-around/evenly; `align-items`/`align-self`/`align-content` (lite); independent `row-gap`/`column-gap` (+ `gap` shorthand); `flex` shorthand; grow/shrink/basis + min-size floor; column path grow/shrink/justify when height definite. **Grid Stage B lite:** `grid-template-columns`/`rows` (`fr`, `repeat`, lengths); independent gaps; `grid-column*` + `grid-row*` spans; 2D occupancy; `justify-items`/`align-items`/`justify-self`/`align-self` (stretch→start). **Not yet:** areas, dense auto-flow, real stretch sizing, subgrid. Paths: `flex.go`, `grid.go`; fixtures 25/28/32; plan `subplans-tier-2/flex-grid-full.md` |
