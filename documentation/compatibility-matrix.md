...
| `background` (shorthand) | Implemented | Color token plus multi-layer background-image `url(...)` / gradients (`background_image.go`). Tests `TestBackgroundImageParse`, `css_partial_remaining_test.go` |
...
- **Not implemented (demoted)** - a row that was previously in the first group; the unread storage and apply arm were removed on 2026-09-12 (PT26-LAY-04), so the declaration is now dropped. Its catalog row is `unsupported`.
...
| `contain` | Not implemented (demoted 2026-09-12) | Was stored at `style.go:348` via `style_advanced_props.go:104`. Values `none|strict|content|size|layout|style|paint|inline-size` parsed but had no layout effect; ponytail audit PT26-LAY-04 removed the unread field and apply arm, so the declaration is now dropped. Re-implement only with a real containment consumer. |
| `contain-intrinsic-size` | Not implemented (demoted 2026-09-12) | Was stored at `style.go:343` via `style_advanced_props.go:88`. Shorthand for the intrinsic placeholder when `contain: size` is active; no layout consumer existed, so PT26-LAY-04 removed the field and apply arm. Declaration is now dropped. |
| `contain-intrinsic-width` | Not implemented (demoted 2026-09-12) | Was stored at `style.go:344` via `style_advanced_props.go:91`. Placeholder inline size with no consumer; field and apply arm removed by PT26-LAY-04. Declaration is now dropped. |
| `contain-intrinsic-height` | Not implemented (demoted 2026-09-12) | Was stored at `style.go:345` via `style_advanced_props.go:94`. Placeholder height with no consumer; field and apply arm removed by PT26-LAY-04. Declaration is now dropped. |
| `contain-intrinsic-inline-size` | Not implemented (demoted 2026-09-12) | Was stored at `style.go:346` via `style_advanced_props.go:97`. Logical placeholder inline size with no consumer; field and apply arm removed by PT26-LAY-04. Declaration is now dropped. |
| `contain-intrinsic-block-size` | Not implemented (demoted 2026-09-12) | Was stored at `style.go:347` via `style_advanced_props.go:100`. Logical placeholder block size with no consumer; field and apply arm removed by PT26-LAY-04. Declaration is now dropped. |
| `content-visibility` | Not implemented (demoted 2026-09-12) | Was stored at `style.go:349` via `style_advanced_props.go:106`. Values `visible|auto|hidden` parsed (initial `visible`); `auto`/`hidden` would skip offscreen rendering in a browser but print renders all content. Field and apply arm removed by PT26-LAY-04; declaration is now dropped. |
...
- `contain` / `contain-intrinsic-*` / `content-visibility` were parsed and stored as containment and content-visibility hints until 2026-09-12, when PT26-LAY-04 removed the unread fields and apply arms; the print engine lays out all boxes eagerly and skips none, so they never had an effect on `Op` output or pagination. Their catalog rows are now `unsupported`.
