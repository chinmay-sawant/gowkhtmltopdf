...
| `text-orientation` (`mixed\|upright\|sideways`) | Implemented (subset) | Vertical writing only: `upright` paints the run at 0 degrees; `sideways` is treated like `mixed` (rotated run). No per-character orientation. Apply arm `style_text_support_props.go:34`; consumer `inline_paint.go:370`. Fixture-62 row 94. |
| `text-combine-upright` (`none\|all\|digits <n>`) | Implemented (subset) | Vertical writing only: a run paints upright when `all` is set or when every character is a digit within the `digits N` cap. No scaled combine cell; mixed digit/letter runs stay rotated. Apply arm `style_text_support_props.go:38`; consumer `inline_paint.go:370/381`. Fixture-62 row 75. |
