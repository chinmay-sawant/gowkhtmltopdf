...
| PDF 1.7 + PDF/A-3a / PDF/UA-1 | **Unreleased 0.2.2** on master (#31, [#45](https://github.com/chinmay-sawant/gowkhtmltopdf/pull/45)/[#47](https://github.com/chinmay-sawant/gowkhtmltopdf/pull/47)); **not** in the 0.2.1 release: opt-in `--pdf-version 1.7` / `WithPDFVersion("1.7")` is a version, not a claim. Profiles `--pdf-profile a3a-ua1` / `a3a` / `ua1` (canonical Get: `PDF/A-3a+PDF/UA-1`, `PDF/A-3a`, `PDF/UA-1`) imply 1.7. Tagged lists nest `L` → `LI` → `LBody` → `Link`. | #31 done |
| PDF 2.0 (ISO 32000-2) | **Unreleased 0.2.2** on master (#32, [#46](https://github.com/chinmay-sawant/gowkhtmltopdf/pull/46)); **not** in 0.2.1: opt-in version via `--pdf-version 2.0` / `WithPDFVersion("2.0")` — header, trailer `/ID`, UTF-8 document strings, non-claiming XMP. Version alone is **not** a PDF/A or PDF/UA claim. | #32 done |
| PDF/A-4 / PDF/UA-2 (PDF 2.0 conformance profiles) | **Unreleased 0.2.2** on master (#33, [#46](https://github.com/chinmay-sawant/gowkhtmltopdf/pull/46)/[#47](https://github.com/chinmay-sawant/gowkhtmltopdf/pull/47)); **not** in 0.2.1: opt-in via `--pdf-profile a4-ua2` / `WithPDFProfile("a4-ua2")` (also `a4`, `ua2`; canonical Get: `PDF/A-4+PDF/UA-2`, `PDF/A-4`, `PDF/UA-2`). Implies PDF 2.0. Emits claiming XMP (`pdfaid:part=4`, `pdfuaid:part=2`), OutputIntent, structure namespaces, and full tagging. | #33 done |
