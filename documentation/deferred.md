...
| WOFF2 / `data:` `@font-face` | **WOFF2 shipped** (local + `https://` via `FetchSub`, then `DecodeWOFF2`). `data:` and EOT still rejected. | `data:` / EOT remain out |
| Fontconfig metric aliases (Gelasio/Cousine-style) | **Not default.** Opt-in `--use-metric-font-aliases` / `UseMetricFontAliases` tries curated accepts (Georgia→Gelasio, …) against registered faces only. No-flag path stays Liberation via the author stack. | Shipped opt-in; must not become no-flag default |
