...
| WOFF2 / `data:` `@font-face` | WOFF2 still skipped in code today (Brotli not yet a direct allowlisted import; `data:` src rejected). Local **and `https://`** TTF/OTF/WOFF1 under ACL/`FetchSub` work. | [`plans/0.2.6/woff2-metric-aliases/`](../plans/0.2.6/woff2-metric-aliases/README.md) (Brotli allowlist amendment + decode); `data:` / EOT remain out |
| Fontconfig metric aliases (Gelasio/Cousine-style) | **Not default.** v0.2.5 resolver continues the author stack to Liberation generics; it does not import host substitutions such as `Georgia → Gelasio`. WeasyPrint differences on the no-flag path are intentional. | Opt-in map planned in [`plans/0.2.6/woff2-metric-aliases/`](../plans/0.2.6/woff2-metric-aliases/README.md) (`--use-metric-font-aliases`); must not become no-flag default |
