...
- [x] 1.7 header + binary comment
- [x] 1.7 xref offsets
- [x] 1.7 trailer `/ID` present (two equal strings on first write); 1.4 trailer unchanged
- [x] 1.7 catalog `/Metadata` present; bytes contain no `pdfaid` / `pdfuaid`
- [x] 1.7 Unicode Info title is UTF-16BE (`FE FF` prefix), not UTF-8
- [x] 1.7 determinism (two writes, equal bytes)
- [x] 1.7 rich document: text + image + link + outline
- [x] Short-writer contract still fails closed on 1.7
- [x] Default `NewDocument` tests still assert `%PDF-1.4`

### 6.2 Semantic parser

- [x] `ParseSemantic` accepts a 1.7 file this package emits — `semantic.go` / `semantic_*_test.go`
- [x] `SemanticDoc.Version` is `"1.7"`
- [x] Page text, images, and annots still populate
- [x] A 1.4 file still reports `"1.4"`

### 6.3 Convert / golden needles

- [x] Add one small committed HTML fixture (or reuse a tiny existing one) converted with version 1.7
- [x] Needle assertions: `%PDF-1.7`, trailer `/ID`, `/Metadata`, Producer contains `1.7`, absence of `pdfaid`
...
- [x] Unsupported version string never produces a file that claims 1.7
- [x] `2.0` still errors (until #32)
- [x] 1.7 + a not-implemented combination errors before `Write`
...
- [x] Do **not** add veraPDF flavour `4` / `ua2` as a #31 gate
