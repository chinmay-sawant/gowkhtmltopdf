...
- [ ] Map `settings.PdfGlobal` version → `pdf.WriterPolicy` in **one** helper (convert or settings adapter). No `if global == "2.0"` in layout
...
- [ ] Test: `Run` / `RunPDF` on a tiny HTML with version 2.0 writes `%PDF-2.0`
...
- [ ] Test: TOC + outline + links job with `--pdf-version 2.0` still opens (structural parse)
