...
- [ ] 87.7.1.1 Record in this file: **Defer all 14** (recommended) or list a tiny experimental subset.
- [ ] 87.7.1.2 Update matrix / deferred notes if the defer is newly explicit for v0.2.7.
- [ ] 87.7.1.3 Leave `mapping.json` `engine_status: unsupported` for all 14 unless a real paint consumer lands.

### 87.7.2 optional experimental subset (only if product overrides defer)

- [ ] 87.7.2.1 Extract apply into `style_border_partial_props.go`.
- [ ] 87.7.2.2 Consumer in `borderOpsSides` for a documented lite (e.g. `border-limit` truncate on straight OpLine only).
- [ ] 87.7.2.3 `border-shape`: treat as **L**; invalid fixture value `bevel` must be fixed to legal syntax before claiming anything.
- [ ] 87.7.2.4 Package tests prove visible geometry change. Flip mapping last with honesty packet.

### 87.7.R batch gate (package only)

- [ ] 87.7.R.1 If deferred: no code change required; check this batch complete when 87.7.1 rows are `[x]`.
- [ ] 87.7.R.2 If experimental code landed: `go test ./internal/layout -run 'TestBorderClip|TestBorderLimit|TestBorderShape' -count=1` exit 0.
- [ ] 87.7.R.3 **No `make test` / `make lint`.**
