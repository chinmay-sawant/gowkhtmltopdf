...
- [ ] 63.1.1 List exact Partial property names owned by this phase (from current `mapping.json`). Proof: names pasted here before coding.

### 63.2 implementation

- [ ] 63.2.1 Implement exit criteria for each owned name. Proof: tests named beside each promotion.

### 63.3 catalog and docs

- [ ] 63.3.1 Flip promoted rows to `implemented` in `catalog/mapping.json`; recount `coverage-summary.json` and `property-counts.md`.
- [ ] 63.3.2 Update `documentation/compatibility-matrix.md` rows to Implemented with honest notes.

### 63.4 gates

- [ ] 63.4.1 Targeted package tests exit 0.
- [ ] 63.4.2 `python3 scripts/css-catalog-map.py --check` exit 0.
- [ ] 63.4.3 Before calling the phase done: `make test` and `make lint` exit 0 (and `make golden` if paint/layout/pagination changed).
