...
Counts from `coverage-summary.json`, refreshed 2026-09-12 by walking `webref-css.json` plus the apply groups under `internal/layout/` (including `style_containment_props.go`, `style_color_adjust_props.go`, `style_image_adjust_props.go`, `style_text_support_props.go`, and `style_font_variant_props.go`). Phase 48.2 reclassified print-noop UI and SVG fill/stroke.

| Kind | Total | implemented | partial | unsupported | ignored |
|------|------:|------------:|--------:|------------:|--------:|
| Properties | 818 | 352 | 2 | 464 | 0 |
...
`unsupported` is the work list. The current catalog snapshot has 464 unsupported and 2 partial properties; `goal: implement` remains set on all 818 inventory rows.
