...
> **Status:** Complete; all 28 items implemented and verified
...
3. **Zero-allocation parsing paths**: High-frequency ASCII scanning routines scan byte-by-byte with zero heap allocations.
4. **Buffer efficiency**: PDF stream operators use `AvailableBuffer()` directly, avoiding format string allocations.
...
- [x] **STYLE-01 · P0 · Safe Style Pointer Resolution**: Replace direct map lookups `e.styles[node].Field` with `e.stylePtr(node)` or `eng.stylePtr(node)` across layout formatting context modules to guarantee that active style overrides are respected and unstyled or synthetic nodes safely return the zero-value fallback without panics.
  - *Location:* [`internal/layout/grid.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/grid.go), [`internal/layout/layout_flow.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/layout_flow.go), [`internal/layout/layout_tables.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/layout_tables.go), [`internal/layout/multicol.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/multicol.go), [`internal/layout/flex.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/flex.go)
  - *Proof:* `go test ./internal/layout/...` (PASS)

- [x] **STYLE-02 · P0 · Sticky Positioning Nil Guard**: Add explicit nil checks on `boxNode.style` in [`internal/layout/sticky.go:21, 69`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/sticky.go#L21) before dereferencing `boxNode.style.Position` or `boxNode.style.Overflow`.
  ...
  - *Proof:* `go test -run TestSticky ./internal/layout/...` (PASS)

### 1.2 Output Generation Escaping & String Correctness
- [x] **STYLE-03 · P0 · Correct Outline XML String Literal**: Fix typo in [`internal/outline/outline.go:380-391`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/outline/outline.go#L380-L391) where `\"/>\node"` and `</item>\node"` contain literal `"node"` characters after `\n`, replacing with clean `\n`.
  ...
  - *Proof:* `go test -run TestDumpOutlineXML ./internal/outline/...` (PASS)

- [x] **STYLE-04 · P0 · HTML Escape Table of Contents Headings**: In [`internal/convert/toc.go:114-135`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/convert/toc.go#L114-L135), escape dynamic heading titles `hVal.Title` and anchors `hVal.Anchor` using `html.EscapeString` when generating TOC HTML markup, preventing malformed DOM trees when headings contain `<`, `>`, or `&`.
  ...
  - *Proof:* `go test -run TestTOCSpecialCharacters ./internal/convert/...` (PASS)

- [x] **STYLE-05 · P1 · CLI Header/Footer Replace Routing**: In [`internal/cli/flags.go:543-561`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/cli/flags.go#L543-L561), update `replaceHF` to inspect both `obj.HeaderSet` and `obj.FooterSet` so `--replace` arguments properly target object footers when only footer flags are configured.
  ...
  - *Proof:* `go test ./internal/cli/...` (PASS)
...
- [x] **STYLE-06 · P2 · Standardize Sentinel Error Prefixes**: Standardize root-re-exported error sentinels in [`internal/errs/errs.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/errs/errs.go#L7-L20) to use the unified module prefix `"gowkhtmltopdf: ..."` matching root API sentinels.
...
- [x] **STYLE-07 · P2 · Direct Use of Canonical Sentinels**: Eliminate unexported error aliases in [`internal/load/load.go:61-75`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/load/load.go#L61-L75) (`errNilLoader`, `errNilContext`) and reference `errs.ErrNilLoader` and `errs.ErrNilContext` directly at call sites.
  ...
  - *Proof:* `go test ./internal/load/...` (PASS)

- [x] **STYLE-08 · P2 · Clean Up Redundant Error Wrapping**: Add descriptive context (`"fetch imported stylesheet %q: %w"`) to error wrapping in [`internal/convert/prepare/styles.go:272`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/convert/prepare/styles.go#L272).
...
- [x] **STYLE-10 · P2 · Image Document Preflight Validation Parity**: Enhance [`ImageDocument.Validate`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/document_validate.go#L83-L109) to validate `Quality` range (0-100) and non-negative `Crop` dimensions before pipeline dispatch, matching the rigor of `Document.Validate`.
  ...
  - *Proof:* `go test -run TestImageDocumentValidate ./...` (PASS)
...
- [x] **STYLE-11 · P1 · Asynchronous Context Watcher in File I/O**: Preserve the context watcher pattern in [`internal/load/load.go:1094-1130`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/load/load.go#L1094-L1130) which closes file descriptors upon context cancellation while guaranteeing goroutine cleanup via `close(stop); <-watcherDone`.
  ...
  - *Proof:* `go test -run TestLoadFileCancellation ./internal/load/...` (PASS)

- [x] **STYLE-12 · P2 · Precompute SSRF Network CIDRs**: Precompute `net.IPNet` instances for CGNAT (`100.64.0.0/10`) and cloud metadata (`169.254.169.0/24`) at package level in [`internal/load/load.go:738-765`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/load/load.go#L738-L765) rather than re-parsing CIDR strings on every IP validation check.
...
- [x] **STYLE-13 · P2 · Cap Supersample Pool Buffer Retention**: In [`internal/imageout/imageout.go:319-354`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/imageout/imageout.go#L319-L354), place an upper capacity bound (e.g. 32 MiB) on buffers returned to `supersamplePixPool` to avoid pinning multi-hundred-megabyte allocations in memory after rendering large rasters.
  ...
  - *Proof:* `go test ./internal/imageout/...` (PASS)
...
- [x] **STYLE-14 · P2 · Avoid By-Value Copies of ResolvedStyle**: In [`internal/layout/inline_collect.go:91-109`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/inline_collect.go#L91-L109), replace `e.styleVal(container)` with `e.stylePtr(container)` during DOM ancestor traversal to avoid copying the ~1.3 KB `ResolvedStyle` struct on every inline node inspection.
...
- [x] **STYLE-15 · P3 · Provide Initial Capacity Hint in Inline Item Pool**: In [`internal/layout/inline.go:158`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/inline.go#L158), allocate `make([]inlineItem, 0, 32)` when `inlineItemPool` is empty, avoiding geometric slice resizes during initial inline collection.
...
- [x] **STYLE-17 · P1 · Split `internal/css/css.go` (1,980 lines)**: Partition along natural seams into:
  ...
  - *Location:* [`internal/css/css.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/css/css.go), [`internal/css/match.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/css/match.go), [`internal/css/selector_parser.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/css/selector_parser.go), [`internal/css/specificity.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/css/specificity.go)
  - *Proof:* `go build ./internal/css/... && go test ./internal/css/...` (PASS)

- [x] **STYLE-18 · P1 · Split `internal/layout/grid.go` (1,891 lines)**: Partition into:
  ...
  - *Location:* [`internal/layout/grid.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/grid.go), [`internal/layout/grid_parse.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/grid_parse.go), [`internal/layout/grid_placement.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/grid_placement.go), [`internal/layout/grid_tracks.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/grid_tracks.go), [`internal/layout/grid_masonry.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/grid_masonry.go)
  - *Proof:* `go build ./internal/layout/... && go test ./internal/layout/...` (PASS)

- [x] **STYLE-19 · P1 · Split `internal/layout/paint_flow.go` (2,368 lines)**: Partition into:
  ...
  - *Location:* [`internal/layout/paint_flow_index.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_flow_index.go), [`internal/layout/paint_flow_breaks.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_flow_breaks.go), [`internal/layout/paint_flow_tables.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_flow_tables.go), [`internal/layout/paint_flow_orphans.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_flow_orphans.go)
  - *Proof:* `go build ./internal/layout/... && go test ./internal/layout/...` (PASS)

- [x] **STYLE-20 · P1 · Split `internal/layout/paint_pagination.go` (2,245 lines)**: Partition into:
  ...
  - *Location:* [`internal/layout/paint_pagination_fixpoint.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_pagination_fixpoint.go), [`internal/layout/paint_pagination_split.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_pagination_split.go), [`internal/layout/paint_pagination_chrome.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_pagination_chrome.go), [`internal/layout/paint_pagination_seal.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_pagination_seal.go)
  - *Proof:* `go build ./internal/layout/... && go test ./internal/layout/...` (PASS)

### 5.2 Linter Cleanliness & Complexity Refactoring
- [x] **STYLE-21 · P2 · Decompose `associateUnmappedOps`**: Refactor [`internal/layout/tagging.go:60-150`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/tagging.go#L60-L150) into specialized sub-functions for link, image, and text ops, eliminating 7 `nolint` suppressions (`cyclop, gocyclo, funlen, gocognit, nestif, varnamelen, wsl`).
  ...
  - *Proof:* `make lint` (PASS)

- [x] **STYLE-22 · P2 · Decompose `beforeAlways`**: Extract the scan state and break event accumulator in [`internal/layout/paint_flow_breaks.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_flow_breaks.go) into a helper struct `breakScanState` to eliminate `//nolint:gocognit,cyclop,funlen`.
  - *Location:* [`internal/layout/paint_flow_breaks.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_flow_breaks.go)
  - *Proof:* `make lint` (PASS)

- [x] **STYLE-23 · P3 · Clean Up Dead Variables and Unused Parameters**:
  - Remove unused `opPage` parameter from `splitCrossingRects` in `internal/layout/paint_pagination_split.go` and `paint.go`.
  - Remove dead assignment `node := loc.Node` and `_ = node` in [`internal/convert/links.go:53`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/convert/links.go#L53).
  ...
  - Remove duplicate doc comment block on `(*hfDrawResult).Err()` in [`internal/convert/hf.go:386`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/convert/hf.go#L386).
  - *Location:* [`internal/layout/paint_pagination_split.go`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/layout/paint_pagination_split.go), [`internal/convert/links.go:53`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/convert/links.go#L53), [`internal/convert/outline.go:137`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/convert/outline.go#L137), [`internal/convert/hf.go:386`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/convert/hf.go#L386)
  - *Proof:* `go test ./... && make lint` (PASS)

- [x] **STYLE-24 · P3 · Implement String and Prefix on Line Severity**: In [`internal/line/line.go:13-41`](file:///home/chinmay/ChinmayPersonalProjects/gowkhtmltopdf/internal/line/line.go#L13-L41), add standard `String()` and `Prefix()` methods on `Severity` and add a nil guard on `writer` in `Emit`.
  ...
  - *Proof:* `go test ./internal/line/...` (PASS)

---

## Phase 6: Verification & Closure Gates

- [x] **STYLE-25 · P0 · Unit and Integration Suite Gate**: Run full test suite across all packages.
  - *Command:* `make test`
  - *Proof:* Exit code 0, all tests pass.

- [x] **STYLE-26 · P0 · Golden Corpus Integrity Gate**: Verify all 61 golden fixtures render with exact structural fidelity, embedded fonts, and page count envelopes.
  - *Command:* `make golden`
  - *Proof:* Exit code 0, 61/61 fixtures pass.

- [x] **STYLE-27 · P0 · Linter Cleanliness Gate**: Verify no lint errors across Go and frontend packages.
  - *Command:* `make lint`
  - *Proof:* Exit code 0, clean output.

- [x] **STYLE-28 · P0 · Product Claims Gate**: Verify no forbidden claims in documentation or CLI help text.
