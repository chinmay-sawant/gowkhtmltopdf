...
- [ ] Record the current resolution path from CSS parsing through
  `layout.lookupFaceFor` and `faceForRune`.
...
- [ ] Record registry sources separately: `--font-path`, `--use-system-fonts`,
  and document `@font-face`.
- [ ] Record the current silent-skip cases for invalid or unsupported font
  files and the current operator log output.
- [ ] Record the current `@font-face` limitation that weight/style
  descriptors are parsed but ignored.
