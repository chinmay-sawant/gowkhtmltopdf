# v0.2.4 Font Resolution, Fallback, and Compliance Plan

> **Status:** planned — validated codebase scan completed 2026-08-18
> **Parent:** [`plans/0.2.4/31-canonical-0.2.4-roadmap.md`](../31-canonical-0.2.4-roadmap.md)
> **Owner:** `internal/pdf`, `internal/layout`, `internal/convert`, CLI and
> public `Document` font options
...
| Named CSS aliases | `FaceSet.ResolveFamily` directly maps names such as Georgia, Times, Arial, and Courier New to bundled families | Can bypass later author-stack entries and cannot produce exact named fonts |
| Optional discovery | `--font-path` and `--use-system-fonts` build a registry from directories; registry matching is by internal family name | The flag is opt-in file discovery, not host Fontconfig alias resolution |
| Layout selection | `lookupFaceFor` checks registry, then bundled faces; per-rune fallback repeats part of the policy | A central resolver seam is needed to avoid divergent family/rune behavior |
| `@font-face` | TTF/OTF/WOFF1 are loaded under the existing ACL; WOFF2/EOT/data are skipped; family is registered as an alias | `font-weight` and `font-style` descriptors are currently ignored |
| Parsing | `ParseTTF` accepts TrueType outlines and rejects CFF/`OTTO`; scanning silently skips parse failures | Supported-format and operator diagnostics need an explicit contract |
| PDF embedding | `ensureFont` subsets and emits simple TrueType or Type0/CIDFontType2 with `FontFile2` and `ToUnicode` | Reuse this pipeline; do not add a second subsetter |
| Compliance | PDF 1.7 + PDF/A-3a/PDF/UA-1 and PDF 2.0 + PDF/A-4/PDF/UA-2 are implemented; default output remains unclaimed PDF 1.4 | New faces must pass the same embed-or-fail and profile gates |
| Public API | `Document.FontPaths` and `Document.UseSystemFonts` already map to `settings.PdfGlobal` | No new public font option is required for the first implementation |
| External engines | Chrome resolves OS-installed Georgia; WeasyPrint follows Linux Fontconfig and selected Gelasio Italic on this host | Differential tests must control the font bytes, not only the CSS family name |
...
optional `Registry`. Its interface should be small enough for layout to ask
for a family/style face and a glyph fallback without knowing whether the face
came from bundled assets, an explicit directory, or document `@font-face`.
...
| 7 | [phase-07-docs-and-operator-contract.md](phase-07-docs-and-operator-contract.md) | Public docs, CLI help, diagnostics, and compatibility claims agree |
...
| Phase 6 accepted visual behavior | Docs and closure |
