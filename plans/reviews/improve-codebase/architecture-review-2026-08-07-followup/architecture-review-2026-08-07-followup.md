...
> **Estimated effort:** 4–8 focused slices, subject to the validation gates below
...
several seams still leak important invariants:
...
The top recommendation is Phase 1: close the mode and output contract. It is a small
deepening with immediate test leverage and reduces ambiguity before larger shared
preparation or display-list work.

## Current architecture rating

**7.9 / 10** — see [rating.md](./rating.md) for the weighted calculation.

This is an architecture rating, not the separate 9.5/10 ponytail leanness rating in
the prior review. The score is held below 9 because the current implementation still
has high-leverage cross-mode seams with inconsistent invariants, despite passing the
current gates.
...
The phase files retain the proposed implementation rows and proof gates.
...
- `[ ]` means proposed or not proven.
- `[x]` means implemented and validated with current evidence.
- `[~]` means intentionally deferred with a reason and next gate.
- No implementation row may become `[x]` before the matching `make lint` and
