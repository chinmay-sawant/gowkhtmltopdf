...
// 7. Check the browser WASM converter surface
console.log('\n7. Checking browser WASM converter assets and route...')
...
const wasmPageSource = readFileSync(join(root, 'src', 'pages', 'WasmPage.jsx'), 'utf8')
const wasmStyles = readFileSync(join(root, 'src', 'styles', 'wasm.css'), 'utf8')
assert.match(appSource, /path=['"]\/wasm['"]/, 'App should register the WASM route')
assert.match(wasmPageSource, /value:\s*['"]png['"]/, 'WASM page should expose PNG output')
assert.match(wasmPageSource, /value:\s*['"]jpeg['"]/, 'WASM page should expose JPEG output')
assert.match(wasmPageSource, /id="wasm-padding"/, 'WASM page should expose image padding')
assert.match(wasmStyles, /:focus-visible/, 'WASM styles should define keyboard focus')
assert.match(wasmStyles, /prefers-reduced-motion/, 'WASM styles should define reduced-motion behavior')
console.log('  ✓ Browser WASM assets, route, and image output options verified.')
