...
Known still-present opportunities as of the mining date (ranked): per-xref-entry formatted write (`writePDFFormat`, `internal/pdf/pdf.go:687`); per-glyph `fmt.Fprintf` at `internal/pdf/fontpdf.go:121`; per-object hashing `fmt.Fprintf` at `internal/pdf/pdf.go:654-666`; per-link formatting at `internal/pdf/pdf.go:1362-1376`; per-ExtGState `fmt.Sprintf` at `internal/pdf/content.go:912-931`; per-W-array-row `fmt.Sprintf` at `internal/pdf/fonttype0.go:236`; per-call `strings.NewReplacer` at `internal/layout/layout_svg.go:221`; one `ReplaceAll` pass per header/footer key at `internal/convert/hf.go:51-57`; per-lookup `regexp.Compile` at `internal/pdf/semantic.go:841-952`; `string(r)` per rune in vertical text at `internal/layout/inline_vertical_writing.go:46-49`.
