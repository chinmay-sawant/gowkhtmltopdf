...
| File / dir | Note |
|------------|------|
| `python/architecture-diagram.pdf` | Regenerated by `make python-api` from `testdata/golden/python_api/architecture-diagram.html` (5 pages) through the Python `Document.pdf()` path. Requires `CGO_ENABLED=1 make c-shared`. |
| `python/invoice-inline.pdf` | Regenerated by `make python-api` via `generate_inline.py`: inline HTML bytes + explicit Document options (typical Python utility shape). |
| `python/pdf-1.7/architecture-diagram.pdf` | Same template with `pdf_version="1.7"` (unclaimed). |
| `python/pdf-1.7-compliance/architecture-diagram.pdf` | Same template with `pdf_profile="a3a-ua1"`. |
| `python/pdf-2.0/architecture-diagram.pdf` | Same template with `pdf_version="2.0"` (unclaimed). |
| `python/pdf-2.0-compliance/architecture-diagram.pdf` | Same template with `pdf_profile="a4-ua2"`. |
| `python/fixture-55-lantern-cooperative-report.pdf` | Manual Python binding smoke (not rewritten by `make samples`). |
| `python/fixture-56-architecture-diagram.pdf` | Manual Python binding smoke of fixture-56 (20 pages; not rewritten by `make samples`). |
